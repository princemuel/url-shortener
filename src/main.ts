import { display, links, linksObjectUpdated } from "./js/display.js";
import { getElement } from "./js/get-element.js";
import { shorten } from "./js/shorten.js";
...
submitBtn.addEventListener("click", () => {
  ...
  if (!links.shortenedLink.length) {
    ...
    const shortened = links.shortenedLink.find(
      (linkDetails) => linkDetails.original_link === link
    );
    if (shortened) {
      const index = links.shortenedLink.findIndex(
        (linkDetails) => linkDetails.original_link === link
      );
      links.shortenedLink = [
